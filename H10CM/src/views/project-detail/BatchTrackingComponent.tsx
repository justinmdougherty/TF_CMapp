...
// --- Helper Functions ---
const getUnitOverallCompletionDate = (
  unit: ProductionUnit,
  projectSteps: ProjectStep[],
): string | undefined => {
  // Need to check if unit is complete using the logic from inside the component
  if (!unit.step_statuses || unit.step_statuses.length === 0) {
    return undefined;
  }

  // A unit is only complete when ALL project steps are accounted for and complete
  for (const projectStep of projectSteps) {
    const stepStatus = unit.step_statuses.find(
      (ss) => ss.stepId.toString() === projectStep.step_id.toString(),
    );

    // If we don't have a status for this step, unit is not complete
    if (!stepStatus) {
      return undefined;
    }

    // If any step is not Complete or N/A, unit is not complete
    if (stepStatus.status !== 'Complete' && stepStatus.status !== 'N/A') {
      return undefined;
    }
  }

  // If we get here, all steps are complete - find the latest completion date
...
const BatchTrackingComponent: React.FC<BatchTrackingComponentProps> = ({ project, steps }) => {
  ...
  const isUnitComplete = (unit: ProductionUnit): boolean => {
    // If there are no step statuses, the unit is not complete
    if (!unit.step_statuses || unit.step_statuses.length === 0) {
      return false;
    }

    // A unit is only complete when ALL project steps are accounted for and complete
    // Check that we have step statuses for all steps in the project
    for (const projectStep of steps) {
      const stepStatus = unit.step_statuses.find((ss) => ss.stepId === projectStep.step_id);
  ...
  const [addUnitsForm, setAddUnitsForm] = useState({
    ...
    attributes: {} as Record<
      string,
      { value: string; isAutoGenerated: boolean; values?: string[] }
    >,
  });
  ...
  const { inProgressUnits, completedUnits, shippedUnits } = useMemo(() => {
    ...
    (trackedItems || []).forEach((unit) => {
      if (unit.is_shipped) {
        ...
        const overallCompletionDate = getUnitOverallCompletionDate(unit, steps);
        comp.push({
          ...unit,
  ...
  const handleOpenAddModal = () => {
    // Initialize attributes based on project attributes
    const attributesForm: Record<
      string,
      { value: string; isAutoGenerated: boolean; values?: string[] }
    > = {};

    projectAttributes?.forEach((attr) => {
      const isAutoGen = attr.is_auto_generated || false; // Use database flag
      const isDate = attr.attribute_type === 'date'; // Use database type

      if (isAutoGen) {
        // Auto-generated attributes get a starting value
        let startValue = '';
        if (attr.display_order === 1) {
          startValue = `${config.snPrefix.unit}${String((trackedItems?.length || 0) + 1).padStart(
            3,
            '0',
          )}`;
        } else if (attr.display_order === 2 && config.snPrefix.pcb) {
          startValue = `${config.snPrefix.pcb}${String((trackedItems?.length || 0) + 1).padStart(
            3,
            '0',
          )}`;
        } else {
          // For other auto-generated fields, create a generic pattern
          const prefix = attr.attribute_name.replace(/\s+/g, '').toUpperCase().substring(0, 3);
          startValue = `${prefix}${String((trackedItems?.length || 0) + 1).padStart(3, '0')}`;
        }
        attributesForm[attr.attribute_definition_id] = { value: startValue, isAutoGenerated: true };
      } else if (isDate) {
        // Date attributes are excluded from batch creation modal
        // They should be handled separately in individual item editing
        return; // Skip date attributes
      } else {
        // Other attributes are manual entry
        attributesForm[attr.attribute_definition_id] = { value: '', isAutoGenerated: false };
      }
    });
  ...
  const handleAddUnitsFormChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (name === 'quantity' || name === 'batchStartDate' || name === 'batchTargetCompletionDate') {
      setAddUnitsForm((prev) => ({
        ...prev,
      ...
      const attributeId = name.replace('attr_', '');
      setAddUnitsForm((prev) => ({
        ...prev,
        attributes: {
          ...prev.attributes,
      ...
      // Handle per-item attribute changes (for quantity > 1)
      const [, itemIndex, attributeId] = name.split('_');
      ...
      setAddUnitsForm((prev) => ({
        ...
        attributes: {
          ...
          [attributeId]: {
            ...prev.attributes[attributeId],
            values: prev.attributes[attributeId].values
              ? prev.attributes[attributeId].values!.map((v, idx) => (idx === itemIdx ? value : v))
              : Array.from({ length: prev.quantity }, (_, idx) =>
                  idx === itemIdx ? value : prev.attributes[attributeId].value,
                ),
          },
        },
      }));
  ...
  const handleAddNewUnits = () => {
    ...
    for (let i = 0; i < addUnitsForm.quantity; i++) {
      ...
      const itemIdentifier = `${config.snPrefix.unit}${String(
        currentTrackedItemsCount + i + 1,
      ).padStart(3, '0')}`;

      // Create initial attributes for this item
      const initialAttributes: Array<{
        attribute_definition_id: number;
        attribute_value: string;
      }> = [];

      // Process all attributes
      Object.entries(addUnitsForm.attributes).forEach(([attributeId, attrData]) => {
        let attributeValue = attrData.value;

        if (attrData.isAutoGenerated) {
          // For auto-generated attributes, increment the value
          const baseValue = attrData.value.replace(/\d+$/, ''); // Remove trailing numbers
          const matches = attrData.value.match(/\d+$/); // Get trailing numbers
          const baseNumber = matches ? parseInt(matches[0]) : 1;
          attributeValue = `${baseValue}${String(baseNumber + i).padStart(3, '0')}`;
        } else if (attrData.values && attrData.values[i]) {
          // For manual attributes with per-item values, use the specific value
          attributeValue = attrData.values[i];
        }
        // For manual attributes without per-item values, use the common value
      ...
      // Create the tracked item with proper structure
      const newItem = {
      ...
      <Dialog open={isAddModalOpen} onClose={handleCloseAddModal} maxWidth="md" fullWidth>
        ...
        <DialogContent sx={{ pt: 2 }}>
          ...
          {projectAttributes?.map((attr) => {
            const attrData = addUnitsForm.attributes[attr.attribute_definition_id];
            if (!attrData) return null;

            const isDate = attr.attribute_type === 'date';
            const fieldType = isDate ? 'date' : 'text';
            const fieldName = `attr_${attr.attribute_definition_id}`;
            const fieldLabel = attrData.isAutoGenerated
              ? `Starting ${attr.attribute_name}`
              : attr.attribute_name;

            return (
              <Box key={attr.attribute_definition_id} sx={{ mb: 3 }}>
                <TextField
                  margin="normal"
                  name={fieldName}
                  label={fieldLabel}
                  ...
                  helperText={
                    attrData.isAutoGenerated
                      ? `Auto-generated: ${attrData.value} → ${attrData.value.replace(
                          /\d+$/,
                          '',
                        )}${String(
                          parseInt(attrData.value.match(/\d+$/)?.[0] || '1') +
                            addUnitsForm.quantity -
                            1,
                        ).padStart(3, '0')}`
                      : !attrData.isAutoGenerated && addUnitsForm.quantity > 1
                      ? 'This value will be applied to all items. Click "Customize per item" to specify different values.'
                      : undefined
                  }
                />

                {/* Per-item customization for manual attributes when quantity > 1 */}
                {!attrData.isAutoGenerated && addUnitsForm.quantity > 1 && (
                  <Box sx={{ mt: 1 }}>
                    <Button
                      size="small"
                        ...
                        setAddUnitsForm((prev) => ({
                          ...
                          attributes: {
                            ...
                            [attr.attribute_definition_id]: {
                              ...prev.attributes[attr.attribute_definition_id],
                              values: Array.from({ length: prev.quantity }, () => attrData.value),
                            },
                          },
                        }));
                      }}
                    ...
                    {attrData.values && (
                      <Box sx={{ ml: 2 }}>
                        {attrData.values.map((value, index) => (
                          <TextField
                            key={index}
                            margin="normal"
                            name={`item_${index}_${attr.attribute_definition_id}`}
                            label={`Item ${index + 1} - ${attr.attribute_name}`}
                            type={fieldType}
                            fullWidth
                            variant="outlined"
                            value={value}
