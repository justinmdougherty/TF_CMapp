// Quick certificate decoder to extract the subject
...
// Decode the certificate using openssl command
const { exec } = require('child_process');
const fs = require('fs');

// Write certificate to temp file
fs.writeFileSync('temp_cert.pem', '-----BEGIN CERTIFICATE-----\n' + DEFAULT_USER_CERT + '\n-----END CERTIFICATE-----');

// Extract subject using openssl
exec('openssl x509 -in temp_cert.pem -noout -subject', (error, stdout, stderr) => {
  if (error) {
    console.error('Error:', error);
    console.log('Fallback: Using hardcoded subject');
    console.log('Subject should be: CN=DOUGHERTY.JUSTIN.MICHAEL.1250227228,OU=USN,OU=PKI,OU=DoD,O=U.S. Government,C=US');
  } else {
    console.log('Certificate subject:', stdout.trim());
  }
...
console.log('Certificate stored in database: CN=DOUGHERTY.JUSTIN.MICHAEL.1250227228,OU=USN,OU=PKI,OU=DoD,O=U.S. Government,C=US');
