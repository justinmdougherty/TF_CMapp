...
async function testCartSystem() {
    try {
        ...
        const inventoryResult = await pool.request().query(`
            SELECT TOP 5 inventory_item_id, part_number, item_name, current_stock_level, cost_per_unit 
            FROM InventoryItems 
            WHERE program_id = 1 
        ...
        inventoryResult.recordset.forEach(item => {
            console.log(`${item.inventory_item_id}: ${item.part_number} - ${item.item_name} (Stock: ${item.current_stock_level}, Cost: $${item.cost_per_unit})`);
        });
        ...
        if (inventoryResult.recordset.length > 0) {
            ...
            const cartItemJson = JSON.stringify({
                ...
                estimated_cost: firstItem.cost_per_unit * 5,
        ...
        if (cartAfterResult.recordset.length > 0) {
            ...
            const orderResult = await pool.request()
                .input('user_id', sql.Int, 2)
                .input('project_id', sql.Int, 1)
                .input('supplier_info', sql.NVarChar, 'Test Supplier')
                .input('order_notes', sql.NVarChar, 'Test order from cart system')
